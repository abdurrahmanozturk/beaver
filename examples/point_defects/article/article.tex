...
The simulation results for different irradiation types are presented by defect concentration, vacancy supersaturation as well as sink strength plots. The calculated sink strengths are also tabulated for different domain size and bias rates.
  ...
  Neutrons have an energy spectrum that varies a couple of orders of magnitude. This makes defect generation possible at different energies. Also, the high penetration depth of neutrons allow them travel through material corresponding to their energy and end up with an uniform or flat-like defect production profiles.
    \subsubsection{Concentrations} \hspace{10pt}
    As a result of uniform defect production and sink at boundaries, the concentration profiles are symmetrical around domain center. For unbiased case, representing the equal amount of point defect production, the steady state value of concentrations are not changing significantly with increasing domain size as shown in Fig. \ref{figure:concentrations_neutron_0_1e-6}. The centerline concentration is sensitive to domain size and might change with increasing domain size. High neutron dose rate raises concentration levels and provides sharper and more stable profiles (see Fig. \ref{figure:concentrations_neutron_0_1e-3}) in the same domain.
