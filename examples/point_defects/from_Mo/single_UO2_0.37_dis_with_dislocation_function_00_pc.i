...
[Functions]
  ...
  [./strain_energy_func]
    type = ParsedFunction
    value = 'xc:=512;yc:=512;q0:=0.3;k:=0.5;n:=1;r:=(((x-xc)^2+(y-yc)^2)^0.5);k*(r^n)+q0'
...
#### added a rec_frac
  ...
  [./rec_frac]
   ...
   function = if(unique_grains>24,1.0/e,0)
    #outputs = exodus
  [../]
[]
...
[UserObjects]
  ...
  [./voronoi]
    ...
    op_num = 8
    grain_num = 25 # All existing grains are assumed to be damaged/deformed
    rand_seed = 81
