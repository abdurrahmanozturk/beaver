...
[Functions]
  [./dist_source]
    type = ParsedFunction
    value = 'k0:=9.037e-18;mu:=5000;sigma:=1000;if(x>mu,k0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5),k0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5)+0.25*k0*(1-pow((x/mu),2)))'
  [../]
[]
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#--------------------------------------------------Kernels-----------------------------------------------
...
[Outputs]
  # exodus = true
  file_base = Nickel_1D_uniform+boundary_sink_1e18_773K_distributed_source_1e-6_30%
  [./exodus]
    type = Exodus
