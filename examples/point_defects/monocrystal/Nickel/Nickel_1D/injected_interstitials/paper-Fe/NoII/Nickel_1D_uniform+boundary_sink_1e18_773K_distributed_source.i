...
[Functions]
  [./dist_source_FP]
    type = ParsedFunction
    value = 'K0:=8.188778358002263e-13;fsurvive:=0.25;ficluster:=0.30;mu:=1000;sigma:=200;if(x>mu,fsurvive*(1-ficluster)*K0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5),fsurvive*(1-ficluster)*K0*exp(-((x-mu+sigma)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5)+0.25*fsurvive*(1-ficluster)*K0*(1-pow((x/mu),2)))'
    # value = 'K0:=9.037e-13;mu:=1000;sigma:=200;K0*exp(-((x-mu)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5)'
  [../]
  [./dist_source_II]
    type = ParsedFunction
    value = 'f_ii:=1.6e-4;K0:=8.188778358002263e-13;fsurvive:=0.25;mu:=1150;sigma:=200;f_ii*fsurvive*K0*exp(-((x-mu)^2)/(2*sigma^2))/(0.4*(2*acos(-1))^0.5)'
  [../]
[]
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#-----------------------------------------------AuxVariables---------------------------------------------
[AuxVariables]
  ...
  [./Dv]    #Vacancy  Diffusion Coefficient {m^2/s}
    initial_condition = 0.0069698724954157755
  [../]
  [./K0]     #Displacement damage rate  {dpa/s}
    initial_condition = 8.188778358002263e-13
  [../]
  ...
  [./Kiv]     #Recombination rate  {1/s}
    initial_condition = 4152.7533557442
  [../]
  [./Kis]     #Sink Reaction rate  {1/s}
    initial_condition = 1.4332e-19
  [../]
  [./Kvs]     #Sink Reaction rate  {1/s}
    initial_condition = 9.562849e-22
  [../]
  [./xie]
    initial_condition = 3.0475025743939657e-22
  [../]
  [./xve]
    initial_condition = 1.8364093649045166e-12
  [../]
  [./super_saturation]
  [../]
...
[ICs]
  [./xv]
    type = RandomIC
    min = 1.6364093649045166e-12     # Nickel equilibrium vacancy concentration
    max = 1.8364093649045166e-12
    variable = xv
  [../]
  [./xi]
    type = RandomIC
    min = 2.0475025743939657e-22
    max = 3.0475025743939657e-22
    variable = xi
  [../]
  # [./xs]
  #   type = RandomIC
...
[Outputs]
  # exodus = true
  file_base = Nickel_1D_uniform+boundary_sink_1e18_773K_distributed_source
  [./exodus]
    type = Exodus
