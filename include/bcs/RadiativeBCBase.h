#ifndef RADIATIVEBCBASE_H
#define RADIATIVEBCBASE_H

#include "IntegratedBC.h"

// Forward Declarations
class RadiativeBCBase;

template <>
InputParameters validParams<RadiativeBCBase>();

class RadiativeBCBase : public IntegratedBC
{
public:
  RadiativeBCBase(const InputParameters & parameters);
  ...
  const Real getArea(const std::vector<Real> & p,
...
protected:
  ...
  std::map<BoundaryID, std::map<BoundaryID, std::map<unsigned int, std::map<unsigned int, Real>>>>
      _viewfactors_map;
  std::map<BoundaryID, std::map<unsigned int, std::map<unsigned int, std::vector<Real>>>>
      _coordinates_map;
