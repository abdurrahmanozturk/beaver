    ...
    _slave_boundary_names(getParam<std::vector<BoundaryName>>("slave_boundary"))
{
  if (_master_boundary_names.size() != 0 && _slave_boundary_names.size() != 0)
  {
    ...
    if (std::find(_master_boundary_names.begin(),
                  _master_boundary_names.end(),
                  "ANY_BOUNDARY_ID") != _master_boundary_names.end())
    {
...
RadiativeBCBase::getMasterBoundaries() const
{
  return _master_boundary_ids;
}

const std::set<BoundaryID> &
RadiativeBCBase::getSlaveBoundaries() const
{
  return _slave_boundary_ids;
}

const Real
RadiativeBCBase::getAngleBetweenVectors(const std::vector<Real> v1,
...
const Real
RadiativeBCBase::getDistanceBetweenPoints(const std::vector<Real> v1,
                                          const std::vector<Real> v2) const
{
  Real d = pow(((v2[0] - v1[0]) * (v2[0] - v1[0]) + (v2[1] - v1[1]) * (v2[1] - v1[1]) +
                (v2[2] - v1[2]) * (v2[2] - v1[2])),
               0.5);
  return d;
}
...
const Real
RadiativeBCBase::getAnalyticalViewFactor(const std::vector<Real> & v)
{
  Real x = 1.0 * v[0] / v[2];
  Real y = 1.0 * v[1] / v[2];
  Real viewfactor =
...
const Real
RadiativeBCBase::getVectorLength(const std::vector<Real> & v) const
{
  return pow((v[0] * v[0] + v[1] * v[1] + v[2] * v[2]), 0.5);
}

const std::map<unsigned int, std::vector<Real>>
RadiativeBCBase::getSideMap(const Elem * elem, const unsigned int side) const
{
...
RadiativeBCBase::getNormal(std::map<unsigned int, std::vector<Real>> map) const
{
  ...
  std::vector<Real> v12{(p2[0] - p1[0]), (p2[1] - p1[1]), (p2[2] - p1[2])};
  std::vector<Real> v13{(p3[0] - p1[0]), (p3[1] - p1[1]), (p3[2] - p1[2])};
...
const std::vector<Real>
RadiativeBCBase::getCenterPoint(std::map<unsigned int, std::vector<Real>> map) const
{
...
const std::vector<Real>
RadiativeBCBase::getRandomDirection(const std::vector<Real> & n, const int dim) const
{
  ...
  Real Rlocal[3][3] = {
      {(cos(theta_local) * cos(phi_local)), sin(phi_local), (-cos(phi_local) * sin(theta_local))},
  ...
  const Real dir_x{sin(theta) * cos(phi)}, dir_y{sin(theta) * sin(phi)}, dir_z{cos(theta)};
  const std::vector<Real> dir_global{dir_x, dir_y, dir_z};
  // transform global direction to local direction
  const std::vector<Real> dir_local{
      (Rlocal[0][0] * dir_global[0] + Rlocal[0][1] * dir_global[1] + Rlocal[0][2] * dir_global[2]),
...
const Real
RadiativeBCBase::getArea(const std::vector<Real> & p,
...
const bool
RadiativeBCBase::isOnSurface(const std::vector<Real> & p,
                             std::map<unsigned int, std::vector<Real>> map) const
{
...
const std::vector<Real>
RadiativeBCBase::getRandomPoint(std::map<unsigned int, std::vector<Real>> map) const
{
  ...
  while (true)
  {
    ...
    const std::vector<Real> p{
        (center[0] + r * dir[0]), (center[1] + r * dir[1]), (center[2] + r * dir[2])};
...
const bool
RadiativeBCBase::isIntersected(const std::vector<Real> & p1,
                               ...
                               std::map<unsigned int, std::vector<Real>> map) const
{
  ...
  const std::vector<Real> p2{(p1[0] + d * dir[0]), (p1[1] + d * dir[1]), (p1[2] + d * dir[2])};
  if (_debugMode == true)
  {
    for (size_t i = 0; i < map.size(); i++) // write nodes to test point is on surface or not
    {
      std::cout << "Slave Node #" << i << " : (" << map[i][0] << "," << map[i][1] << ","
                << map[i][2] << ")" << std::endl;
    }
...
const bool
RadiativeBCBase::isSidetoSide(
    const std::map<unsigned int, std::vector<Real>> & master_side_map,
    const std::map<unsigned int, std::vector<Real>> & slave_side_map) const
{
  ...
  for (size_t i = 0; i < master_side_map.size(); i++)
  {
    ...
    for (size_t j = 0; j < slave_side_map.size(); j++)
    {
      ...
      const std::vector<Real> master_slave = {(slave_node[0] - master_node[0]),
                                              (slave_node[1] - master_node[1]),
                                              (slave_node[2] - master_node[2])};
      const std::vector<Real> slave_master = {(master_node[0] - slave_node[0]),
                                              (master_node[1] - slave_node[1]),
                                              (master_node[2] - slave_node[2])};
...
const bool
RadiativeBCBase::isVisible(const std::map<unsigned int, std::vector<Real>> & master_side_map,
                           const std::map<unsigned int, std::vector<Real>> & slave_side_map) const
{
...
void
RadiativeBCBase::printViewFactors()
{
  std::cout << " " << std::endl;
    ...
    for (const auto & slave_boundary : master_boundary_map)
    {
      ...
      for (const auto & master_elem : slave_boundary_map)
      {
        ...
        std::cout << "\tElem " << master_elem.first << "  --->  "
                  << "Bnd " << slave_boundary.first
                  << "\t  Total View Factor = " << elem_to_bnd_viewfactor << "\n"
                  << std::endl;
...
const Real
RadiativeBCBase::doMonteCarlo(std::map<unsigned int, std::vector<Real>> master_side_map,
