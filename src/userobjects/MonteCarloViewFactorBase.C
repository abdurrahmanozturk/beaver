...
template <>
InputParameters
validParams<MonteCarloViewFactorBase>()
{
...
MonteCarloViewFactorBase::MonteCarloViewFactorBase(const InputParameters & parameters)
...
MonteCarloViewFactorBase::getMasterBoundaries() const
{
  return _master_boundary_ids;
}

const std::set<BoundaryID> &
MonteCarloViewFactorBase::getSlaveBoundaries() const
{
  return _slave_boundary_ids;
}

const std::map<unsigned int, std::vector<Point>>
MonteCarloViewFactorBase::getSideMap(const Elem * elem,const unsigned int side) const
{
...
const Point
MonteCarloViewFactorBase::getNormal(std::map<unsigned int, std::vector<Point>> map) const
{
...
const Point
MonteCarloViewFactorBase::getCenterPoint(std::map<unsigned int, std::vector<Point> > map) const
{
...
const Real
MonteCarloViewFactorBase::getArea(const Point &p, std::map<unsigned int, std::vector<Point>> map) const
{
...
const Point
MonteCarloViewFactorBase::getRandomDirection(const Point & n,const int dim) const
{
...
const bool
MonteCarloViewFactorBase::isOnSurface(const Point &p, std::map<unsigned int, std::vector<Point>> map) const
{
...
const Point
MonteCarloViewFactorBase::getRandomPoint(std::map<unsigned int, std::vector<Point>> map) const
{
...
const bool
MonteCarloViewFactorBase::isIntersected(const Point & p1,
                              const Point & dir,
...
const bool
MonteCarloViewFactorBase::isSidetoSide(const std::map<unsigned int, std::vector<Point>> & master_side_map,
...
const bool
MonteCarloViewFactorBase::isVisible(const std::map<unsigned int, std::vector<Point>> & master_side_map,
                          const std::map<unsigned int, std::vector<Point>> & slave_side_map) const
{
...
void
MonteCarloViewFactorBase::printViewFactors()
{
  std::cout << " " << std::endl;
  ...
  for (const auto & master_boundary : _viewfactors_map)
  {
    ...
    for (const auto & slave_boundary : master_boundary_map)
    {
      ...
      for (const auto & master_elem : slave_boundary_map)
      {
        ...
        //           << "Bnd " << slave_boundary.first << "\t  Total View Factor = " <<
        //           elem_to_bnd_viewfactor
        //           << "\n"<<std::endl;
      }
...
const Real
MonteCarloViewFactorBase::doMonteCarlo(std::map<unsigned int, std::vector<Point>> master_side_map,
